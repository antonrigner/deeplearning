...
def runModel(learning_rate):
    ...
    for fold in range(no_folds):
        ...
        plt.show()
    cross_val_error = np.mean(cross_val_error)
    print('The cross validation error for model with learning parameter ' + str(learning_rate) + ' : ' + str(cross_val_error))
...
def main():
    ...
    for learning_rate in learningRateSS:
        cross_val_error = runModel(learning_rate)
        cross_val_errors.append(cross_val_error)
#    plt.figure(2)
#    plt.plot(learningRateSS, cross_val_errors, marker = 'x', linestyle = 'None')
#    plt.xticks(learningRateSS)
#    plt.xlabel('Learning rates')
#    plt.ylabel('mean error cross validation errors')
#    plt.show()
    
    print(learningRateSS[np.argmin(cross_val_errors)])
    fig = plt.figure(2)
    ax1 = fig.add_subplot(211)
    ax1.set_title('Mean cross alidation error')
    ...
    # Plot test errors against training epochs
